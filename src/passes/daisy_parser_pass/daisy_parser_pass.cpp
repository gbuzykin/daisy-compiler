        ...
        if (act != parser_detail::predef_act_shift) {
            ...
            if (logger::g_debug_level >= 3) {
                if (tt == parser_detail::tt_id) {
                    logger::debugUnwind(la_tkn_.loc).format("id: {}", std::get<std::string>(la_tkn_.val));
...
void daisy::logSyntaxError(int tt, const SymbolLoc& loc) {
    ...
    switch (tt) {
        ...
        case parser_detail::tt_end_of_line: msg = "unexpected end of line"; break;
