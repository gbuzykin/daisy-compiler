...
int main(int argc, char** argv) {
    ...
    for (const auto& path : test_lst) {
        ...
        try {
            ...
            std::string sout;
            std::copy(std::istreambuf_iterator<char>{if1}, std::istreambuf_iterator<char>{}, std::back_inserter(sout));
            if1.close();
            ...
            if (!update) {
                ...
                if (!if2) { throw std::runtime_error("cannot open file `" + path + ".out.expected" + "` for reading"); }
                ...
                if (!of2) { throw std::runtime_error("cannot open file `" + path + ".out.expected" + "` for writing"); }
